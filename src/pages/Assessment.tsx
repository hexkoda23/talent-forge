...
import { ArrowRight, Brain, Lock, Puzzle, ShieldCheck, Timer, Trophy } from "lucide-react";
...
const games = [
  ...
  { id: "speed", name: "Speed Reasoning", desc: "Quickfire decisions. Do not let the clock win.", icon: Timer, time: "~2 min", color: "accent" },
];
  ...
  <div className="min-h-screen flex flex-col">
    ...
    <div className="flex-1 px-5 py-10 lg:py-16 max-w-5xl w-full mx-auto">
      <div className="text-center max-w-2xl mx-auto mb-10 animate-fade-up">
        <p className="text-sm font-mono uppercase tracking-widest text-secondary mb-3">// game stage</p>
        <h1 className="font-display text-4xl lg:text-5xl font-bold">Cognitive Assessment</h1>
        <p className="text-muted-foreground mt-4">
          Three short games measure memory, logic, and speed reasoning. A high score marks you as game-qualified, but admin verification is still required before admission.
        </p>
      </div>
      ...
      <div className="glass-panel rounded-2xl p-6 lg:p-8 text-center">
        ...
        <p className="text-muted-foreground text-sm mb-6 max-w-md mx-auto">
          You will play all three back-to-back. If you arrived here without completing identity verification, start there first.
        </p>
        <div className="flex flex-wrap justify-center gap-3">
          <Button variant="hero" size="xl" className="gap-2" asChild>
            <Link to="/assessment/play">Start assessment <ArrowRight className="h-5 w-5" /></Link>
          </Button>
          <Button variant="soft" size="xl" className="gap-2" asChild>
            <Link to="/register"><ShieldCheck className="h-5 w-5" /> Verify identity</Link>
          </Button>
        </div>
        <div className="mt-6 flex items-center justify-center gap-2 text-xs text-muted-foreground">
          <Lock className="h-3.5 w-3.5" />
          Game qualification is private and always reviewed with your identity file.
        </div>
      </div>
    </div>
  </div>
